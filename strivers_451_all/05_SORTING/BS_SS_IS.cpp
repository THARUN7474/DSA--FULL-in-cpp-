...
void bubblesort_DSC(int arr[], int n)
{
    bool is_swap;
    for (int i = 0; i < n - 1; i++)
    {                    // Outer loop should run n-1 times
        is_swap = false; // Reset is_swap for each outer loop iteration
        for (int j = 0; j < n - i - 1; j++)
        { // Inner loop should start from 0
            if (arr[j] < arr[j + 1])
            { // For descending order, swap if arr[j] < arr[j+1]
                swap(arr[j], arr[j + 1]);
                is_swap = true;
            }
        }
...
void selectionsort_DSC(int arr[], int n)
{
    ...
    for (int i = 0; i < n - 1; i++)
    {
        ...
        {
            if (arr[si] < arr[j])
            { // If a larger element is found, update si
                si = j;
            }
        }
...
void selection_sort_rec(int arr[], int i, int n) {
